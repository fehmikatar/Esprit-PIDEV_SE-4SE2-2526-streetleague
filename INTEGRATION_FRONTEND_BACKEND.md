...
## Nouvelles fonctionnalités (Team, Community, Category)

### 1) Team: Scheduler métier (backend)

- Un scheduler Spring exécute une maintenance automatique chaque nuit.
- Règle métier: toute équipe ACTIVE, sans membres, et créée depuis plus de 60 jours est ARCHIVED.
- Impact base de données: mise à jour bulk sur la table teams.

Configuration optionnelle (application.properties):

```properties
# Exécution tous les jours à 02:00
app.team.scheduler.archive-cron=0 0 2 * * *
```

### 2) Community: Fonction complexe JPQL avec JOINs

Endpoint backend:

- `GET /api/communities/insights?categoryKeyword=fit&minMembers=2`

Comportement:

- JPQL avec JOIN entre `Community`, `Category`, `CommunityMember` (et jointure utilisateur côté membres).
- Agrégation avec `COUNT` des membres par communauté.
- Filtrage par mot-clé catégorie + seuil minimum de membres.
- Résultat trié par popularité (nombre de membres).

Exemple Angular (service):

```typescript
getCommunityInsights(categoryKeyword = '', minMembers = 0) {
  return this.http.get<CommunityDto[]>(
    `${environment.apiUrl}/api/communities/insights`,
    { params: { categoryKeyword, minMembers } }
  );
}
```

Exemple Angular (composant):

```typescript
this.communityApi.getCommunityInsights('football', 3).subscribe({
  next: (rows) => this.communityRows = rows,
  error: (err) => console.error('Community insights error', err)
});
```

### 3) Category: Fonction complexe par keywords (>1 table)

Endpoint backend:

- `GET /api/categories/search/keyword?keyword=ball`

Comportement:

- Requête Spring Data par keywords: recherche sur le nom de catégorie **ou** le nom produit associé.
- Implique plusieurs tables (`categories` + `products`).
- Retourne les catégories distinctes.

Exemple Angular (service):

```typescript
searchCategoriesByKeyword(keyword: string) {
  return this.http.get<CategoryDto[]>(
    `${environment.apiUrl}/api/categories/search/keyword`,
    { params: { keyword } }
  );
}
```
...
```typescript
this.categoryApi.searchCategoriesByKeyword(this.searchTerm).subscribe({
  next: (rows) => this.filteredCategories = rows,
  error: (err) => console.error('Category keyword search error', err)
});
```

### 4) Team: Endpoint de recherche multi-table (bonus)

Endpoint backend:

- `GET /api/teams/search/by-category-owner?category=foot&owner=ali`

Comportement:

- Requête par keywords sur catégorie + prénom du créateur d'équipe.
- Requête dérivée Spring Data impliquant `teams`, `categories`, `user`.

Exemple Angular:

```typescript
searchTeams(category: string, owner: string) {
  return this.http.get<Team[]>(
    `${environment.apiUrl}/api/teams/search/by-category-owner`,
