...
public class PromoCodeService implements IPromoCodeService {
    ...
    public List<PromoCodeDTO> addPromoCodes(List<PromoCodeDTO> promoCodeDTOs) {
        return promoCodeDTOs.stream().map(this::addPromoCode).collect(Collectors.toList());
    }

    @Override
    public PromoCodeDTO updatePromoCode(Long id, PromoCodeDTO promoCodeDTO) {
        Optional<PromoCode> optional = promoCodeRepository.findById(id);
        if (optional.isEmpty()) return null;
        PromoCode promoCode = optional.get();
        if (!promoCode.getCode().equals(promoCodeDTO.getCode()) && promoCodeRepository.existsByCode(promoCodeDTO.getCode())) {
            return null;
        }
    ...
    public PromoCodeDTO getPromoCodeById(Long id) {
        return promoCodeRepository.findById(id).map(promoCodeMapper::toDTO).orElse(null);
    }

    @Override
    public PromoCodeDTO getPromoCodeByIdOrElse(Long id) {
        PromoCode defaultCode = PromoCode.builder()
                .id(0L).code("DEFAULT").discountType(PromoCode.DiscountType.PERCENTAGE)
                .discountValue(BigDecimal.ZERO).active(false).createdAt(LocalDateTime.now()).build();
        PromoCode promoCode = promoCodeRepository.findById(id).orElse(defaultCode);
        return promoCodeMapper.toDTO(promoCode);
    }

    @Override
    public PromoCodeDTO getPromoCodeByCode(String code) {
        return promoCodeRepository.findByCode(code).map(promoCodeMapper::toDTO).orElse(null);
    }

    @Override
    public List<PromoCodeDTO> getAllActivePromoCodes() {
        return promoCodeRepository.findByActiveTrue().stream().map(promoCodeMapper::toDTO).collect(Collectors.toList());
    }
    ...
    public boolean isValidPromoCode(String code) {
        return promoCodeRepository.findByCode(code).map(PromoCode::isValid).orElse(false);
    }
    ...
    public PromoCodeDTO applyPromoCode(String code, BigDecimal amount) {
        return promoCodeRepository.findByCode(code)
                ...
                .map(pc -> {
                    BigDecimal discounted = pc.applyDiscount(amount);
                    PromoCodeDTO dto = promoCodeMapper.toDTO(pc);
                    dto.setDiscountValue(discounted);
                    return dto;
                }).orElse(null);
    }
    ...
    public void deactivateExpiredPromoCodes() {
        List<PromoCode> expired = promoCodeRepository.findExpiredActivePromoCodes(LocalDateTime.now());
        expired.forEach(pc -> pc.setActive(false));
